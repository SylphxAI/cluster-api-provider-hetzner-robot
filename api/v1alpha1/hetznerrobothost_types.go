...
// +kubebuilder:validation:XValidation:rule="!has(oldSelf.serverIP) || size(oldSelf.serverIP) == 0 || (has(self.serverIP) && self.serverIP == oldSelf.serverIP)",message="serverIP is immutable once populated"
// +kubebuilder:validation:XValidation:rule="!has(oldSelf.serverIPv6Net) || size(oldSelf.serverIPv6Net) == 0 || (has(self.serverIPv6Net) && self.serverIPv6Net == oldSelf.serverIPv6Net)",message="serverIPv6Net is immutable once populated"
// +kubebuilder:validation:XValidation:rule="!has(oldSelf.internalIP) || size(oldSelf.internalIP) == 0 || (has(self.internalIP) && self.internalIP == oldSelf.internalIP)",message="internalIP is immutable once populated"
// +kubebuilder:validation:XValidation:rule="!has(oldSelf.installDisk) || size(oldSelf.installDisk) == 0 || (has(self.installDisk) && self.installDisk == oldSelf.installDisk)",message="installDisk is immutable once populated"
