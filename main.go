...
func main() {
	...
	if err = (&controllers.HetznerRobotHostReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "HetznerRobotHost")
		os.Exit(1)
	}

	if err = (&controllers.HetznerRobotRemediationReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "HetznerRobotRemediation")
		os.Exit(1)
	}
